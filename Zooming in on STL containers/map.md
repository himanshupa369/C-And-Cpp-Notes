<img width="1685" height="707" alt="Screenshot 2025-12-23 143337" src="https://github.com/user-attachments/assets/12fd2f19-f5c3-4eda-bba3-c09b1a39cf68" />
<br>
---
...
* Introduced in **C++98**
* Continually enhanced in later standards (C++11, C++17, C++20)

---

## **Definition**

`std::map` is an **associative container** that stores elements as **explicit key–value pairs**, where:

* Each **key is unique**
* Each key maps to **exactly one value**
* Elements are **automatically ordered by key**

Each element internally behaves like:
...
2. **Keys are unique** (no duplicate keys allowed)
3. Elements are **ordered by key**
4. Ordering is done using:

   * `std::less<Key>` by default (ascending order)
5. **Fast lookup** (logarithmic time)
6. Similar properties to `std::set`, but stores values alongside keys
...
```cpp
std::map<int, int> m = {{2,20}, {0,10}, {1,15}};
```
...
| `Key`        | Type of key                      |
| `Value`      | Type of value                    |
| `Comparator` | Optional key comparison function |
...
* `bool == true` → insertion successful
* `bool == false` → duplicate key
...
* Keys can be **any comparable type**:
  ...
  * `int`
  * `std::string`
  * Custom objects (must support comparison)

---

## **Custom Ordering (Comparators)**

### **1. Default Comparator**

```cpp
std::map<int, int> m; // Uses std::less
```
...
* Fast lookup
* Strong key-value association
* No duplicate keys
* Flexible ordering with custom comparators

---

## **Cons**

* Slower than `unordered_map` for simple lookups
...
* Ordered datasets requiring fast search

---

## **Important Notes**

* Keys are **const** once inserted
* Values **can be modified**
* Ordering is **always based on keys**
* Requires a valid comparison operator for key type

---

## **Summary**

* `std::map` stores **unique keys with associated values**
* Data is **automatically ordered by key**
* Supports **multiple traversal styles**
* Comparator can be customized using:

  * Functors
  * Function pointers
  * Lambdas
* Ideal when **ordering + fast lookup** is required
